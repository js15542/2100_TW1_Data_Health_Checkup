...
def check_consistency(df: pd.DataFrame) -> tuple[dict, dict[str, pd.DataFrame]]:
    # Literal labels only: "Yes" / "No" / "1 (ambiguous)". The 50 numeric rows are
    # never merged into "Yes" here (that is a cleaning decision, rule R4).
    lbl = df["Churn_Label"].astype(str).str.strip()
    churn = lbl.where(lbl.str.lower().isin(["yes", "no"]), "1 (ambiguous)")
    ...
    miss_by_churn = pd.crosstab(churn, miss, normalize="index").rename(columns={True: "missing_share", False: "present_share"}).round(3).reset_index()
...
def part2_part3_support(df: pd.DataFrame) -> tuple[dict, dict[str, pd.DataFrame]]:
    # RAW view: per unique Customer_ID, but no other cleaning. "Churned" means the
    # literal text "Yes"; rows coded "1" are reported as "1 (ambiguous)" and are NOT
    # counted as churned here. tw1_raw_vs_clean_metrics.py shows every number below
    # side by side with the cleaned-table version.
    d = df.drop_duplicates("Customer_ID").copy()
    lbl = d["Churn_Label"].astype(str).str.strip()
    d["churn"] = lbl.where(lbl.str.lower().isin(["yes", "no"]), "1 (ambiguous)")
    ...
    leak = pd.crosstab(d["churn"], d["Date_Cancelled"].notna().map({True: "Date_Cancelled present", False: "Date_Cancelled blank"}))
    ...
    summary = {
        ...
        "ambiguous_label_rows_excluded": int((d["churn"] == "1 (ambiguous)").sum()),
        ...
        "leakage_churn_yes_total": int((d["churn"] == "Yes").sum()),
        "leakage_churn_no_with_date": int(((d["churn"] == "No") & d["Date_Cancelled"].notna()).sum()),
        "leakage_churn_no_total": int((d["churn"] == "No").sum()),
        "leakage_ambiguous_with_date": int(((d["churn"] == "1 (ambiguous)") & d["Date_Cancelled"].notna()).sum()),
        "leakage_ambiguous_total": int((d["churn"] == "1 (ambiguous)").sum()),
    }
    tables = {
...
Part 1 uses the delivered raw extract without cleaning. Part 2 uses one row per Customer_ID and title-cases Industry only for grouping; these reporting-only transformations are identified below.
...
- Churn label vs. cancel date: {l['churn_yes_without_date_cancelled']} rows coded as churned (Yes or 1) without a Date_Cancelled (all of them are the numeric "1" rows); {l['churn_no_with_date_cancelled']} non-churned rows with a Date_Cancelled; {l['date_cancelled_before_join_date']} rows cancelled before they joined.
...
- Pricing sanity: typical accounts pay about **${c['revenue_per_user_typical_median']} per user per month** (range {c['revenue_per_user_typical_range']}). The whale row implies ${c['whale_revenue_per_user']} per user and the 50,000-user row implies ${c['extreme_users_revenue_per_user']} per user, i.e. both are roughly 1,000x off the pricing pattern. This strongly suggests data-entry errors, possibly an extra "000", but the source system should be checked before correction. {c['rows_outside_revenue_per_user_band']} rows fall outside the ${REV_PER_USER_LOW}-${REV_PER_USER_HIGH} per-user band (sheet `Consist_RevPerUser`).
...
- Support_Tickets blanks are not strongly concentrated in the checked groups: missing share is {c['support_missing_share_churned_vs_retained']} for churned vs. retained customers and {c['support_missing_share_by_industry_range']} across industries. This supports simple imputation plus a missing flag instead of dropping rows.
...
- Note: these are RAW-view numbers: one row per Customer_ID, Industry title-cased for grouping only, churned = literal "Yes" ({p['ambiguous_label_rows_excluded']} rows coded "1" are excluded, not merged). The by-year denominator is customers who had joined before Jan 1 of that year and had not yet cancelled. For the same numbers on the cleaned table (and the Raw-vs-Clean comparison teammates should cite), run `tw1_raw_vs_clean_metrics.py`.

## Part 3 support - Data Leakage evidence
- Evidence from the raw labels (one row per Customer_ID, no other cleaning; the raw file has 486 `Yes` rows, two of which are duplicates): Date_Cancelled is present in **{p['leakage_churn_yes_with_date']} of {p['leakage_churn_yes_total']}** rows labelled `Yes` and in **{p['leakage_churn_no_with_date']} of {p['leakage_churn_no_total']}** rows labelled `No`; among the {p['leakage_ambiguous_total']} rows coded `1`, {p['leakage_ambiguous_with_date']} have a date and {p['leakage_ambiguous_total'] - p['leakage_ambiguous_with_date']} do not. Wherever the label is unambiguous, the column reproduces it exactly, and a cancellation date can only exist after the customer has left. Date_Cancelled is therefore a direct proxy for the outcome and must be dropped from the modelling features. See sheet `P3_LeakageCrosstab`.
...
- `tw1_raw_vs_clean_metrics.py` - Parts 2-5 numbers, Raw and Clean side by side (`TW1_Raw_vs_Clean_Metrics.xlsx` / `.md`).
"""
